...
class FlappyBird(base.PyGameWrapper):
    ...
    def getGameState(self):
        ...
        """
        state = np.array([self.player.pos_y / (0.79*self.height - self.player.height),
            (next_pipe.x - self.player.pos_x) / (self.width),
            next_pipe.gap_start / (0.79*self.height),
            (next_pipe.gap_start + self.pipe_gap) / (0.79*self.height),
            (next_next_pipe.x - self.player.pos_x) / (2*self.width),
            (next_next_pipe.gap_start) / (0.79*self.height),
            (next_next_pipe.gap_start + self.pipe_gap) / (0.79*self.height)])
